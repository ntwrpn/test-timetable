...
public class LessonController {
    ...
    public ResponseEntity<List<Lesson>> getLessonPage(Model model) {
        List<Lesson> orders = lessonService.getAll();
        return new ResponseEntity<List<Lesson>>(orders, HttpStatus.OK);
    }
    
    @RequestMapping(value="/lesson/", method=RequestMethod.OPTIONS)
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity getLessonKeys(Model model) {
        return new ResponseEntity(lessonService.getFields(), HttpStatus.OK);
    }
    
    @RequestMapping(value="/lesson/{id}", method=RequestMethod.GET)
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity<Lesson> getLessonPage(Model model, @PathVariable("id") UUID id) {
        Optional<Lesson> order = lessonService.getById(id);
        return new ResponseEntity<Lesson>(order.get(), HttpStatus.OK);
    }
    ...
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity<Void> update(@PathVariable("id") UUID id, @RequestBody Lesson obj){
     obj.setId(id);
    ...
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity<Void> DeleteLesson(Model model, @PathVariable UUID id) {
