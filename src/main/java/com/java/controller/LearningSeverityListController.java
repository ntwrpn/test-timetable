...
public class LearningSeverityListController {
    ...
    public ResponseEntity<List<LearningSeverityList>> getLearningSeverityListPage(Model model) {
        List<LearningSeverityList> orders = learningSeverityListService.getAll();
    ...
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity getLearningSeverityListKeys(Model model) {
    ...
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity<LearningSeverityList> getLearningSeverityListPage(Model model, @PathVariable("id") UUID id) {
        Optional<LearningSeverityList> order = learningSeverityListService.getById(id);
        return new ResponseEntity<LearningSeverityList>(order.get(), HttpStatus.OK);
    }

    @RequestMapping(value="/learningseveritylist/", method = RequestMethod.POST, headers="Accept=application/json")
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity<Void> add(@RequestBody LearningSeverityList obj){
    ...
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity<Void> update(@PathVariable("id") UUID id, @RequestBody LearningSeverityList obj){
     obj.setId(id);
    ...
    @PreAuthorize("hasRole('ROLE_USER') or hasRole('ROLE_ADMIN')")
    public ResponseEntity<Void> DeleteLearningSeverityList(Model model, @PathVariable UUID id) {
