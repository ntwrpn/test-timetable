...
const deleteValueFromTable = (event) => {
    ...
    $.ajax({
        ...
        'type': 'POST',
        'url': "deleteuseremployee" + "?EmployeeId=" + id,
        'success': function (response) {
            M.toast({html: "Связь удалена"});
            openTableEvent("table-form", "data-tr-table", "urn:jsonschema:com:java:domain:Employee");
...
const acceptValueFromTable = (event) => {
    let id = event.currentTarget.value;
    let url = "addusertoemployee/?userId=" + id + "&EmployeeId=" + localStorage.getItem("employee");
    $.ajax({
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
...
const opentableinform = (event) => {
    localStorage.setItem("employee", event.currentTarget.value)
...
$(document).ready(function () {
    ...
    $(".sidenav").sidenav();
    $('.modal').modal();
    openTableEvent("table-form", "data-tr-table1", "urn:jsonschema:com:java:domain:Employee");
});
